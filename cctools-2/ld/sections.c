...
unsigned long *nextrel)
{
	...
	for(i = 0; i < map->s->nreloc; i++){
		 ...
		 * defined as an N_ABS symbol it will will turn r_symbolnum
		...
		if(r_symbolnum != NO_SECT &&
		   ...
		   r_pcrel == 1){
		    ...
		    else if(arch_flag.cputype == CPU_TYPE_MMIX)
			mmix_reloc(fake_contents, fake_relocs, &fake_map);
#ifndef RLD
...
struct section_map *map)
{
	...
	if(output_for_dyld){
	    ...
	    /*
	     * For indirect sections only those relocation entries for items in
	     * the section used from this object will be saved.  So allocate a
	     * buffer to put them in to use to do the relocation and later
	     * scatter_copy_relocs() will pick out the the relocation entries to
	...
	if(arch_flag.cputype == CPU_TYPE_MC680x0)
	    ...
	    mmix_reloc(contents, relocs, map);
#ifndef RLD
	else if(arch_flag.cputype == CPU_TYPE_I860)
...
#ifdef DEBUG
	    /*
	     * count_reloc() counted and recorded the number of relocation 
	     * entries the section was to have in the output.  This should
	     * match what reloc_output_for_dyld() copied out.
	     */
	    if((map->s->flags & SECTION_TYPE) == S_REGULAR ||
	       (map->s->flags & SECTION_TYPE) == S_MOD_INIT_FUNC_POINTERS ||
	       (map->s->flags & SECTION_TYPE) == S_MOD_TERM_FUNC_POINTERS){
	...
	if((map->s->flags & SECTION_TYPE) == S_NON_LAZY_SYMBOL_POINTERS ||
	   (prebinding == TRUE &&
		...
		if(map->fine_relocs[i].use_contents == TRUE){
			...
			if(map->fine_relocs[i].local_symbol == FALSE){
...
/*
printf("Need to adjust INDIRECT_SYMBOL_LOCAL value = 0x%x\n", value);
*/
...
*/
	...
	/*
	 * For other indirect sections only copy those parts of the section
	 * who's contents are used in the output file.
	 */
...
unsigned long *nextrel)
{
    ...
    unsigned long r_address, r_extern, r_type, r_scattered, r_pcrel,
		  r_symbolnum, r_value;
    enum bool indirect_section, pic;
