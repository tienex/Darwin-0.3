...
merge_symbols(void)
{
	...
	if(filetype == MH_DYLIB){
		...
		error_with_cur_obj("object can't contain both private external "
		    "symbols and global symbols with MH_DYLIB output file");
...
void)
{
    unsigned long i, j, k, l, nundefineds;
	...
	for(i = 0; i < cur_obj->dylib_module->nextdefsym; i++){
		...
		/*
		 * The symbol exist.  So if the symbol is anything but a common
		 * or undefined then it is multiply defined.
		 */
		merged_symbol = *hash_pointer;
		if(merged_symbol->nlist.n_type != (N_UNDF | N_EXT)){
		    /*
	    ...
	    merged_symbol->nlist.n_sect = NO_SECT;
	    /*
	     * don't change the n_desc field as it contains the reference
	     * type (lazy or non-lazy).
	     */
...
output_local_symbols(void)
{
	...
	for(i = 0; i < cur_obj->symtab->nsyms; i++){
		 ...
		 * indirect sections check to see if this symbol was one that
	...
	if(nmerged_private_symbols != 0 && keep_private_externs == FALSE){
		...
		for(i = 0; i < merged_symbol_list->used; i++){
			...
			if(merged_symbol->definition_object != cur_obj){
			    ...
			    index = cur_obj->iprivatesym;
			}
			merged_symbol->output_index = index++;
		    }
		}
	    }
	}
...
void)
{
	...
	for(q = &objects; *q; q = &(object_list->next)){
		...
		mod->iinit	= cur_obj->iinit;
		mod->ninit	= cur_obj->ninit;
#ifdef DYLIB_TERM
		/* TODO: still need to check somewhere for overflow */
		mod->iinit	= (cur_obj->iterm << 16) | cur_obj->iinit ;
		mod->ninit	= (cur_obj->nterm << 16) | cur_obj->ninit;
#endif
