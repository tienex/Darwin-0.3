...
enum bool time_stamps_must_match)
{
	...
	/* if a root_dir option is used prepend the directory for rooted names*/
...
good:
	...
	if(time_stamps_must_match == TRUE){
		...
		if(lc->cmd == LC_ID_DYLIB){
			...
			error("library: %s (architecture %s) prebinding not "
			      "uptodate with dependent dynamic shared library: "
			      "%s", file_name, arch_name, dylib_name);
			exit(3);
		    }
		}
		lc = (struct load_command *)((char *)lc + lc->cmdsize);
	...
	/*
	 * Add this library's ofile to the list of libraries the current arch
	 * depends on.
	 */
	libs = reallocate(libs, (nlibs + 1) * sizeof(struct lib));
	memset(libs + nlibs, '\0', sizeof(struct lib));
	libs[nlibs].dylib_name = (char *)dl_load + dl_load->dylib.name.offset;
...
void)
{
	...
	/* figure out what this arch's seg1addr is */
